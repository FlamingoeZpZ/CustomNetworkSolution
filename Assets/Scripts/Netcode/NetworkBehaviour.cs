...
namespace Netcode
{
    public abstract class NetworkBehaviour : MonoBehaviour
    {
        ...
        public int RequiredUpdateLength { get; private set; }
        //protected byte[] UploadBuffer;
        //protected byte[] PreviousBuffer;
        //protected byte[] Buffer;
    
        protected abstract int InitializeBuffers();
        ...
        protected virtual void OnEnable()
        {
            ...
            if (!networkObject)
            {
                Debug.LogWarning("No networkobject attached from, to: " + transform.root + " --> " + name);
                enabled = false;
                return;
            }

            networkObject.OwnershipChanged += OnOwnershipChanged; 
            RequiredUpdateLength = InitializeBuffers();
            networkObject.AddToUpdateSet(this);
            print($"{name} is {RequiredUpdateLength} bytes long to update.");
        ...
        {
            if(!networkObject ) networkObject = GetComponentInParent<NetworkObject>();
            networkObject.RemoveFromUpdateSet(this);
            networkObject.OwnershipChanged -= OnOwnershipChanged;
        }
     ...
     //   }
        ...
        public static T NetworkInstantiate<T>(T toSpawn, Vector3 initialLocation, Quaternion initialRotation, ulong ownerID) where T : NetworkBehaviour
        {
            ...
            //Assign ID.
            networkObject.AssignId(ownerID);
